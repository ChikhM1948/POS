...
import type { CreateCustomerInput } from '@pos-dz/shared';
...
export function CustomerForm({ onSubmit, onCancel }: Props) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('customerForm.errorSave'));
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 rounded-xl border border-neutral-200 bg-white p-5 shadow-card">
      <h3 className="text-base font-semibold text-neutral-900">{t('customerForm.title')}</h3>
      ...
      <div className="flex gap-2">
        <button disabled={submitting} type="submit" className="rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white shadow-card transition hover:bg-brand-700 disabled:cursor-not-allowed disabled:opacity-50">
          {submitting ? t('common.saving') : t('customerForm.submit')}
        </button>
