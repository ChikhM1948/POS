...
const ProductSchema = new Schema<IProduct>(
  {
    ...
    // Saisi librement par produit (ProductForm) ; recalculé en moyenne pondérée à chaque réception
    // fournisseur (voir stock.service.recomputeCostPrice) — pas un ledger à part : contrairement au
    // stock, une valeur "en cache, recalculable" suffit ici et évite de rejouer tout StockMovement
    // pour connaître le coût courant à la caisse (y compris hors-ligne).
    costPriceCents: { type: Number, default: 0, min: 0 },
    minSellingPriceCents: { type: Number, min: 0 },
